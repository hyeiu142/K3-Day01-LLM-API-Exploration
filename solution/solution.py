...
def format_comparison_table(results: list[dict]) -> str:
    ...
    """
    header = f"{'Prompt':<40} | {'GPT-4o Response':<40} | {'Mini Response':<40} | {'4o Latency':<10} | {'Mini Latency':<12}\n"
    divider = "-" * 152 + "\n"
    table = header + divider
    for r in results:
        p = r["prompt"][:37] + "..." if len(r["prompt"]) > 40 else r["prompt"]
        g = r["gpt4o_response"][:37] + "..." if len(r["gpt4o_response"]) > 40 else r["gpt4o_response"]
        m = r["mini_response"][:37] + "..." if len(r["mini_response"]) > 40 else r["mini_response"]
        gl = f"{r['gpt4o_latency']:.2f}s"
        ml = f"{r['mini_latency']:.2f}s"
        table += f"{p:<40} | {g:<40} | {m:<40} | {gl:<10} | {ml:<12}\n"
    return table
